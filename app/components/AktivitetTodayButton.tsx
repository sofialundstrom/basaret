...
export function AktivitetTodayButton() {
  ...
  return (
    <div className="flex justify-center">
      ...
      >
        <span className="shrink-0 rounded-lg border-2 border-nollebok-ink bg-nollebok-lila px-2.5 py-1 text-xs font-bold uppercase tracking-wide md:text-sm">
          Idag
        </span>
        <span className="min-w-0 flex-1">
          <span className="block text-sm font-bold md:text-base">{label}</span>
          <span className="mt-1 block text-xs text-nollebok-ink/70 md:text-sm">
            Tryck här för dagens aktiviteter
          </span>
        </span>
        <span aria-hidden className="shrink-0 text-xl leading-none md:text-2xl">
          ↓
        </span>
      </button>
    </div>
